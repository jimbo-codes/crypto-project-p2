...
function CryptoData({name,price,dodChg, mktCap, mktCapRank, id, maxSupply, volume, index, image, setSelected}) {
...
return (      
      ...
      <td className="px-0 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><img className="float-left w-6 pr-1" src={image} alt={name}/><Link to={`/app/${id}`} onClick={()=>setSelected(true)}>{name}</Link></td>
      ...
      <td><Link to={`/app/${id}`} onClick={()=>setSelected(true)} className="relative inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Inspect</Link></td>
